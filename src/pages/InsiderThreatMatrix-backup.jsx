...
const InsiderThreatMatrix = () => {
  ...
  const changeView = (newView, options = {}) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
    }, 150); // Brief loading state for better UX
  };
  ...
  const getSmartRecommendations = useMemo(() => {
    ...
    const criticalThreats = stats.allThreats.filter(t => 
      ['Data Loss', 'Exfiltration via Web Service', 'Espionage', 'Financial Misappropriation'].includes(t.title)
    );
    ...
    if (uncoveredCritical.length > 0) {
      recommendations.push({
        ...
        action: () => {
          // Show first uncovered critical threat
